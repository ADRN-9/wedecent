...
// TestStagingRoutedTerminalAdversarialDestinationRouterTrust verifies that a
// valid staging-issued route capability and explicit B->C router trust do not
// let an untrusted router enter C's route-tunnel role. C->B destination-router
// trust is independent from all other route and terminal trust domains.
// Because B verifies and consumes the single-use capability before dialing C,
// the trusted retry intentionally uses a freshly issued capability after C->B
// trust is installed.
func TestStagingRoutedTerminalAdversarialDestinationRouterTrust(t *testing.T) {
	...
	if _, ok := cRuntime.DestinationRouters.Get(bID.ID); ok {
		t.Fatal("destination router trust was inferred instead of explicitly configured")
	}
	...
	secondDestinationErr := serveOneRouteTunnel(ctx, cListener, cRuntime)
	secondRouterErr := serveOneRouteControl(ctx, bListener, bRuntime)
	conn, err = dialer.Dial(ctx, "")
	if err != nil {
		t.Fatalf("fresh route capability failed after explicit destination->router trust: %v", err)
	}
	if err := conn.Close(); err != nil {
		t.Fatal(err)
	}
	if err := waitStagingRuntime(ctx, secondRouterErr); err != nil {
		t.Fatalf("router runtime after destination trusted router: %v", err)
	}
	if err := waitStagingRuntime(ctx, secondDestinationErr); err != nil {
		t.Fatalf("destination runtime after explicit router trust: %v", err)
	}

	t.Log("WEDECENT_STAGING_ROUTED_TERMINAL_ADVERSARIAL_DESTINATION_ROUTER_TRUST_OK")
}
