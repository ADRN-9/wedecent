...
func NewServer(status v1.StatusService, devices v1.DeviceService) (*Server, error) {
	...
	return &Server{status: status, devices: devices}, nil
}

// ServeOne handles exactly one framed request and writes exactly one framed
// response. It deliberately does not listen on a socket or authenticate a local
// caller; the eventual platform IPC adapter must provide that same-user boundary.
